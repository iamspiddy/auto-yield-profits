...
DROP FUNCTION IF EXISTS log_kyc_action(TEXT, UUID, TEXT);
DROP FUNCTION IF EXISTS verify_kyc(UUID, UUID);
DROP FUNCTION IF EXISTS reject_kyc(UUID, UUID, TEXT);
...
-- Add indexes for KYC queries (IF NOT EXISTS to avoid conflicts)
...
DROP POLICY IF EXISTS "Admins can view all profiles for KYC" ON public.profiles;
DROP POLICY IF EXISTS "Admins can update profiles for KYC" ON public.profiles;
